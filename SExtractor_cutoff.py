...
def modify_sentences(json_path, api_key=DEFAULT_API_KEY):
    ...
    for key, (value, byte_reduction) in data.items():
        # 计算需要减少的汉字数量
        char_reduction = byte_reduction // 2
        sentence = value
            ...
            while retries < 3:  # 允许最多尝试3次（包括第一次）
                request_data = {
                    ...
                    'messages': [
                        ...
                        {'role': 'user', 'content': f'请帮我将以下中文句子缩短{char_reduction}个字，可以缩短或删除部分汉字或标点符号："{sentence}"'}  # 用户请求缩短句子的指令
                    ],
                    'stream': False
                }
                ...
                if response.status_code == 200:
                    ...
                    break
            if retries == 3:
                print(f"重试失败：{key} 缩短尝试{retries}次失败，未能成功缩短句子，跳过此条目。")
        # else:
